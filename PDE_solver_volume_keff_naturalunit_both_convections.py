...
def update_fin(dictionary, rows, columns, step_size, k, forced, wind_speed = 20,
                length_scale = 0.001):
    # Precompute common values
    if forced == False:
        power_factor = 4 / 3
...
def update_sink(matrix, rows, columns, step_size, k, forced, wind_speed = 20,
                 length_scale = 0.001):
    # Precompute common values
    if forced == False:
        power_factor = 4 / 3
...
def power_out_fins(fin_meshes, step_size, forced, k_M = 150, 
                   length_scale = 0.001, wind_speed = 20):
...
def sink_top_power_out(matrix_S, column_num_F, separation_num, step_size, 
                       forced, k_M = 150, length_scale = 0.001, wind_speed=20):
    power_out = 0
...
def progress_update(iter_num, progress_interval, iteration, step_size, k_eff_CM, 
                    ...
                    separation_num, mean_temp_M, forced, length_scale = 0.001, 
...
def create_plot(matrix_M, matrix_C, matrix_S, fin_meshes, fin_separation, 
    ...
    im1 = ax1.imshow(big_mesh*293, cmap='viridis', interpolation='nearest')
    ax1.set_title('Heatmap for EVERYTHING (K)')
...
def optimized_iteration(iteration,  num_fins, row_num_M, fin_separation, 
    ...
    progress_interval = max(iteration // 20, 1)
    ...
    for i in range(iteration):
        ...
        progress_update(iter_num = i, progress_interval = progress_interval, 
                        ...
                        separation_num = int(fin_separation / 
                                             (step_size*length_scale)), 
    ...
    return mean_temp_M, mean_temp_C, mean_temp_S, step_size