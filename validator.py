...
def create_plot(matrix_M):
    
    M = matrix_M[1:-1, 1:-1]
    M *= 293
    print("Maximum ", max(M[int(len(M)/2)]), ", Minimum ", min(M[int(len(M)/2)]))
    ...
    plt.tight_layout()
    plt.plot(np.linspace(0, 14, 70), M[int(len(M)/2)], '.')
    plt.savefig('/Users/alanli/Desktop/Comp Phys Proj/Submission_folder/temprange.png', dpi=500)
    plt.show()

def create_vert_plot(matrix_M, matrix_C):
    ...
    plt.ylabel('Temperature (K)')
    plt.tight_layout()
    plt.plot(np.linspace(0, 3, 60), big_mesh[:,100], '.')
...
def solve(row_num_M, iteration = 50000000, k_M = 150, k_C = 230, q = 5e8, 
    ...
    for i in range(iteration):
        ...
        if np.abs(power_frac_diff) < convergence and np.abs(temp_frac_diff) < convergence:
            ...
            break
    if show_plot == True:
        create_plot(matrix_M)
    
    if show_vert_plot == True:
        create_vert_plot(matrix_M, matrix_C)
    if temp_track == True:
